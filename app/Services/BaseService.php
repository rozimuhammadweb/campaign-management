    ...
    public function filter(Builder $query, $filter_fields, $params)
    {
        foreach ($filter_fields as $key => $item) {
            if (array_key_exists($key, $params)) {
                ...
                if ($item['type'] == 'intarray' && $params[$key] && !in_array(0, $params[$key])) {
                ...
                if ($item['type'] == 'date' && $params[$key] != '')
                    $query->whereDate($key, '=', $params[$key]);
                
                if ($item['type'] == 'datefrom' && $params[$key] != '')
                {
                    $column = str_replace("_from", "", $key);
                    $query->whereDate($column, '>=', $params[$key]);
                }
                
                if ($item['type'] == 'dateto' && $params[$key] != '')
                {
